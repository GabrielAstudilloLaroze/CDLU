...
nivel <- read_delim("~/Canvas Data/nivel.csv", ";", escape_double = FALSE, 
...
enrollment_dim <- read_delim("~/Canvas Data/datos/enrollment_dim.txt", 
...
quiz_dim <- read_delim("~/Canvas Data/datos/quiz_dim.txt", 
                       "\t", escape_double = FALSE, col_names = TRUE, 
                       col_types = cols(id = col_character(), 
...
quiz_submission_fact <- read_delim("~/Canvas Data/datos/quiz_submission_fact.txt",
                                   "\t", escape_double = FALSE, col_names = TRUE, 
...
user_dim <- read_delim("~/Canvas Data/datos/user_dim.txt", 
                       "\t", escape_double = FALSE, col_names = TRUE, 
                       col_types = cols(id = col_character()), 
                       trim_ws = TRUE)
...
account_dim <- read_delim("~/Canvas Data/datos/account_dim.txt", 
...
course_dim <- read_delim("~/Canvas Data/datos/course_dim.txt", 
                         "\t", escape_double = FALSE, col_names = FALSE, 
                         col_types = cols(X1 = col_character(), 
...
#base: sections.
course_section_dim <- read_delim("~/Canvas Data/datos/course_section_dim.txt", 
                                 "\t", escape_double = FALSE, col_names = FALSE, 
                                 col_types = cols(X1 = col_character()), 
                                 trim_ws = TRUE)
...
module_dim <- read_delim("~/Canvas Data/datos/module_dim.txt", 
                         "\t", escape_double = FALSE, col_names = TRUE, 
                         col_types = cols(id = col_character(),
...
module_item_dim <- read_delim("~/Canvas Data/datos/module_item_dim.txt", 
                              "\t", escape_double = FALSE, col_names = TRUE, 
                              col_types = cols(id = col_character(),
                                               canvas_id = col_character(),
...
assignment_dim <- read_delim("~/Canvas Data/datos/assignment_dim.txt", 
                             "\t", escape_double = FALSE, col_names = TRUE, 
                             col_types = cols(id = col_character(),
                                              canvas_id = col_character(),
                                              course_id = col_character()),
...
enrollment_dim<-enrollment_dim%>%
  filter(!(user_id == "user_id estudiante retirado 1" |
             user_id == "user_id estudiante retirado n..."))

#Agregar estudiantes que no figuran.
enrollment_dim$workflow_state[enrollment_dim$user_id=="user_id estudiante faltante"]<-"active"
...
# Para situaciones que se salgan del modelo evaluativo, se debe identificar el quiz_id,
# e imputar tipo de evaluación que corresponda, de acuerdo a la nomenclatura
# del bloque de código anterior.


evaluaciones$ce[(evaluaciones$quiz_id == "quiz_id evaluación inconsistente 1")]<-"Control"
evaluaciones$ce[(evaluaciones$quiz_id == "quiz_id evaluación inconsistente 2")]<-"PDN"



##Desde aquí PROCESO QUE CUENTA Y NUMERA POR ORDEN EVALUACIONES.
...
######################################################################################
#
#                                 CONFIGURACIÓN CICLOS EVALUATIVOS
#
######################################################################################

# Los ciclos evaluativos se definen por la fecha de inicio de los controles (15 preguntas)
# y término de las pruebas.
...
######################################################################################
# Para los controles, los ensayos simce y de pruebas de acceso universitario, se utiliza
#  el criterio del orden de publicación a través del semestre, correspondiente a la variable 
# n_eval, la cual identifica el orden de publicación según el tipo de evaluación y dentro de la
# asignatura. De esta manera los ensayos simce no interfieren la cuenta de controles, como
# tampoco una asignatura interfiere con otra.


# Para las pruebas de nivel se utiliza el ciclo evaluativo. es la Única manera de ordenarlas adecuadamente.

#Hay asignaturas en los que los controles requieren organizarse según el ciclo evaluativo.
######################################################################################

evaluaciones<-evaluaciones %>%
  mutate(orden = case_when(evaluaciones$ce=="PDN" ~ as.numeric(ciclo),
                           evaluaciones$ce=="Control" ~ n_eval,
                           course_id == "course_id caso especial 1" & evaluaciones$ce=="Control"~ as.numeric(ciclo),
                           evaluaciones$ce=="Ensayo Simce" ~ n_eval,
                           evaluaciones$ce=="Ensayo PDT" ~ n_eval,
                           course_id == "course_id caso especial 2" & evaluaciones$ce=="Control"~ as.numeric(ciclo)))
...
# Si por alguna razón hay controles que aparezcan con un orden equivocado, 
# se pueden corregir mediante el quiz_id, y reemplazar "99" por el valor numérico
# que le corresponda.



evaluaciones$orden[(evaluaciones$quiz_id == "quiz_id control con orden equivocado 1")]<-99
evaluaciones$orden[(evaluaciones$quiz_id == "quiz_id control con orden equivocado 2")]<-99
...
## Chequear que correspondan las referencias de hojas de cálculo.
