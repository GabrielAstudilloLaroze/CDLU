...
subtitle: Liceo Industrial Presidente Pedro Aguirre Cerda 
output:
  pdf_document:
    toc: yes
---
...
colegio<-"Liceo Industrial Presidente Pedro Aguirre Cerda"
...
``` {R echo=FALSE, include = TRUE}
  ...
  geom_boxplot()+
  labs(title = "Gráfico 6: Resultados Prueba de Nivel Tercer Año",
       subtitle = "Por Asignatura de Formación General.")+
  coord_flip()
...
bd_fd<-col%>%
  filter(Nivel == "III",  
           `Asignatura o módulo` == "AJUSTE DE MOTORES" |
`Asignatura o módulo` == "AUTOMATIZACIÓN DE SISTEMAS ELÉCTRICOS INDUSTRIALES" |
`Asignatura o módulo` == "CONTROL DE PROCESOS INDUSTRIALES" |
`Asignatura o módulo` == "CUBICACIÓN - CARGUÍO Y TRANSPORTE" |
`Asignatura o módulo` == "FORTIFICACIÓN DE MINAS SUBTERRÁNEA Y A CIELO ABIERTO" |
`Asignatura o módulo` == "Fresado, Taladrado y Rectificado de Piezas y Conjuntos Mecánicos - Cuarto Medio"|
...
`Asignatura o módulo` == "LECTURA DE PLANOS DE CUBICACIÓN Y TRONADURA" |
...
`Asignatura o módulo` == "MANTENIMIENTO Y REPARACIÓN INDUSTRIAL" |
`Asignatura o módulo` == "MECÁNICA DE BANCO"|
  `Asignatura o módulo` == "Mecanizado con Máquinas de Control Numérico Computacional - Cuarto Medio" |
  `Asignatura o módulo` == "MONTAJE DE EQUIPOS Y SISTEMAS INDUSTRIALES" |
  `Asignatura o módulo` == "MUESTREO DE EXPLOTACIONES MINERAS" |
  `Asignatura o módulo` == "PERFORACIÓN Y TRONADURA EN FAENAS MINERAS" |
  `Asignatura o módulo` == "SOLDADURA INDUSTRIAL" |
  `Asignatura o módulo` == "Torneado de Piezas y Conjuntos Mecánicos - Cuarto Medio" |
  `Asignatura o módulo` == "VENTILACIÓN SECUNDARIA Y DRENAJE DE MINAS")%>%
  select(enrollment_rollup_id,Nivel,Sección,`Asignatura o módulo`)
...
pr_fd$`Asignatura o módulo`<-str_replace_all(pr_fd$`Asignatura o módulo`, " - Cuarto Medio", "")
pr_fd$`Asignatura o módulo`<-str_replace_all(pr_fd$`Asignatura o módulo`, "MANTENIMIENTO DE LOS SISTEMAS DE ", "M. SIST.")
pr_fd$`Asignatura o módulo`<-str_replace_all(pr_fd$`Asignatura o módulo`, "SISTEMAS", "S.")
pr_fd$`Asignatura o módulo`<-str_replace_all(pr_fd$`Asignatura o módulo`, "Mecanizado con Máquinas de Control Numérico Computacional", "Mecanizado...")
...
\centering
Tabla 9: % de Participación y logro en prueba de nivel 5. Tercer año, Formación de Especialidad.
``` {R echo=FALSE, include = TRUE}
kable(part_fd1,"pipe")
```
...
``` {R echo=FALSE, include = TRUE}
  ...
  ggplot(part_fd1.1, aes(`Asignatura o módulo`,`% Logro`))+
  geom_boxplot()+
  labs(title = "Gráfico 7: Resultados Prueba de Nivel\nTercer Año",
       subtitle = "Por módulo de Formación de Especialidad")+
  coord_flip()
...
\centering
Tabla 10: % de Participación y logro en prueba de nivel 5. Cuarto año, Formación de Especialidad
``` {R echo=FALSE, include = TRUE}
kable(part_fd1,"pipe")
```
...
``` {R echo=FALSE, include = TRUE}
  ggplot(part_fd1.1, aes(`Asignatura o módulo`,`% Logro`))+
  geom_boxplot()+
  labs(title = "Gráfico 8: Resultados Prueba de Nivel\n Cuarto Año",
       subtitle = "Por módulo de Formación de Especialidad")+
  coord_flip()
