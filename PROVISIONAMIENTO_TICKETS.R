...
ourse_dim <- read_delim("~/Canvas Data/datos/course_dim.txt", 
                         "\t", escape_double = FALSE, col_names = FALSE, 
                         col_types = cols(X1 = col_character(), 
...
#Si es que hay discordancias en el estado de los estudiantes, este bloque de código lo corrige.
...
#aquí se pueden incorporar manualmente mediante su user_id.
...
#Si es que hay estudiantes que no figuran en algún curso, aquí se puede actualizar manualmente su estado
#mediante user_id

enrollment_dim$workflow_state[enrollment_dim$user_id=="#user_id estudiante faltante 1"]<-"active"
enrollment_dim$workflow_state[enrollment_dim$user_id=="#user_id estudiante faltante n..."]<-"active"




#Si es que hay tickets de salida que no aparecen, aquí se puede corregir
quiz_dim$points_possible[quiz_dim$id == "quiz_id ticket faltante"]<-5


##El siguiente bloque de texto crea un contador de tickets de salida de acuerdo al orden de publicación dentro de la asignatura.
mq<-module_item_dim%>%
  filter(!(quiz_id == "\\N"))%>%
...
#Definir semestre. En este caso, se tomó como referencia la semana 22. Reemplazar si es necesario.
t_date$semestre<-NA
...
## Si se quiere filtrar solo el segundo semestre, uncomment el comando filter.
...
#Si es que todavía existen, eliminar "estudiantes de prueba"
data_est <- data_est%>%
...
#Si se quiere filtrar solo segundo semestre, unccoment semestre.
tickets<-tickets%>%
  filter(!(is.na(nivel))
         #,semestre == 2
         )
...
###EXPORTAR A GOOGLESHEET.
library(googlesheets4)

gs4_auth()
1 #Si es necesario, reemplazar por el a que haya quedado asignada la cuenta que se va a utilizar.
...
#Chequear consistencia de referencias en hojas de cálculo de google de CADA LICEO.
...
#Revisar si es que hay modificaciones en los nombres de módulos de especialidad.
...
col_especialidad<-tickets%>%
  filter(Colegio== "Liceo Industrial de Angol",`Asignatura o módulo` == "AJUSTE DE MOTORES" |
           ...
           `Asignatura o módulo` == "MECANIZADO CON MÁQUINAS DE CONTROL C" |
...
##NO INCLUYE TALLERES ARTÍSTICOS.
